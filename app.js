...
function detectSigns(landmarks) {
    ...
    const now = Date.now();
    // Debounce to prevent rapid fire of the same sign
    if (now - lastSignTime < 2000) return null;

    // 1. Detect "Stop / Wait" (Open hand, all fingers extended far above wrist)
    if (indexTip.y < wrist.y - 0.25 && middleTip.y < wrist.y - 0.25 && ringTip.y < wrist.y - 0.2 && pinkyTip.y < wrist.y - 0.2) {
        // If not waving, it's an open palm
        return "Stop";
    }

    // 2. Detect "Yes" (Fist, all finger tips below their corresponding MCP joints)
    const indexMcp = landmarks[5];
    const middleMcp = landmarks[9];
    if (indexTip.y > indexMcp.y && middleTip.y > middleMcp.y && ringTip.y > landmarks[13].y) {
        return "Yes";
    }

    // 3. Detect Waving (Hello)
...
function onResults(results) {
    ...
    if (results.rightHandLandmarks) {
        ...
        const sign = detectSigns(results.rightHandLandmarks);
        if (sign) {
